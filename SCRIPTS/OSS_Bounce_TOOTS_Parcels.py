...
from parcels import FieldSet, ParticleSet, JITParticle, ScipyParticle, AdvectionRK4, DiffusionUniformKh, Variable, Field, GeographicPolar, Geographic, ErrorCode, plotTrajectoriesFile, NestedField
...
file_name="OSS_07262023_oocysts_model_2019.zarr"
path2 = '/home/pi/wongalaj/Ciannelli_Lab/wongalaj/TOOTS/OSS_07262023_oocysts_model_2019.nc' # give path to where .nc file is
feather_path = 'OSS_07262023_oocysts_model_2019.feather' # assign name of feather file
...
file1 = 'https://pae-paha.pacioos.hawaii.edu/erddap/griddap/roms_hiomsg' # OAHU SS
...
# ds1
# myDat1 = ds1.sel(**{'time': slice(startDate,endDate)}) # subset out specific time period based on defined start and end date

# myDat1
myDat1 = ds1.isel(**{'depth': simDepthOSS}).sel(**{'time': slice(startDate,endDate)}) # subset out specific depth layer

U1=Field.from_xarray(myDat1.u, 'U', dimensions, allow_time_extrapolation=True)
V1=Field.from_xarray(myDat1.v, 'V', dimensions, allow_time_extrapolation=True)
...
## 3. Generate fieldset
fieldset2 = FieldSet(U, V)


## Nested: Define U and V for the MHI ROMS

Umhi = fieldset2.U
Vmhi = fieldset2.V


## Nest fine res and MHI ROMS together


UNested = NestedField('U', [Ufineres, Umhi])
VNested = NestedField('V', [Vfineres, Vmhi])
...
### Code to make landmask (MHI and OAHU SS)
...
### Import velocity field for OAHUSS ROMS

# OAHUSS ROMS 

file_path1 = '/nfs7/CEOAS/Ciannelli_Lab/wongalaj/TOOTS/roms_hiomsg_2018_05_05_ffdb_1c31_10ba.nc' 
...
### Make landmask for each hi res and MHI ROMS

##### "land = 1" and "ocean = 0"

landmask_fineres = make_landmask1(file_path1) # OAHU SS
...
### Nested model: add landmask and distance to shore
...
fieldset.add_field(Field('landmask_coarse', landmask_mhi,
                         lon=fieldset.U[1].grid.lon, lat=fieldset.U[1].grid.lat,
                         mesh='spherical'))
...
### ALL MODELS: Add eddy diffusivity
...
# ## Nested boundaries: OAHU SS

fieldset.add_constant('min_lon', -158.1218)
#add max lon of fine
fieldset.add_constant('max_lon', -157.7903)
#add min lat of fine
fieldset.add_constant('min_lat', 21.23194)
#add max lat of fine
fieldset.add_constant('max_lat', 21.32355)
...
### Oocysts Model: Create for loop to release number of particles based on oocysts hydrological model output
